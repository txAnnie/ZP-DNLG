...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, inputs, memory_bank, pro, src_pad_mask, tgt_pad_mask,
        ...
        if VERSION == 2:
            pro_mid, pro_attn = self.context_attn(pro, pro, query_norm,
                                                  mask=src_pad_mask,
                                                  layer_cache=layer_cache,
                                                  type="context")
            if pro_bmm:
                pro_mid = pro_attn.bmm(pro)

            output = self.feed_forward(self.drop(mid) + self.drop(pro_mid) + query)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory_bank, pro, state, memory_lengths=None,
        ...
        emb = self.embeddings(tgt, step=step) # (len, batch, dim)
        ...
        outputs = output.transpose(0, 1).contiguous() # (len, batch, dim)
        ...
        return outputs, state, attns, emb
