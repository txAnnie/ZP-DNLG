...
class LossComputeBase(nn.Module):
    ...
    def sharded_compute_loss(self, batch, output, attns, cur_trunc, trunc_size, shard_size,
                             normalization, s_loss=None, g_loss=None):
        ...
        for shard in shards(shard_state, shard_size):
            ...
            loss_ = loss.div(float(normalization))
            if s_loss:
                loss_ = loss_ + SIM_Rate * s_loss
                s_loss = None
            if g_loss:
                loss_ = loss_ + GEN_Rate * g_loss
                g_loss = None
            loss_.backward(retain_graph=True)
            batch_stats.update(stats)

            # break  # 改了
        return batch_stats, loss_
