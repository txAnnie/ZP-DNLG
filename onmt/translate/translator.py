...
class Translator(object):
    ...
    def _translate_batch(self, batch, data):
        ...
        enc_states, memory_bank, pro, pro_pre = self.model.encoder(src, src_lengths, False)  # second:add False
        if TEST_:
            pro_predict.append(pro_pre.cpu().tolist())
        dec_states = self.model.decoder.init_decoder_state(
        ...
        if isinstance(memory_bank, tuple):
            ...
            if pro is not None:
                pro = rvar(pro.transpose(0, 1).data).transpose(0, 1)
        ...
        for i in range(self.max_length):
            ...
            dec_out, dec_states, attn, _ = self.model.decoder(
                inp, memory_bank, pro, dec_states,
                memory_lengths=memory_lengths,
                step=i)
        ...
        if TEST_:
            save_data(pro_predict, pro_path)
        return ret
