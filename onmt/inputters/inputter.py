...
from onmt.utils.file_util import *


pro_list = ['我', '我们', '你', '你们', '他', '她', '它', '他们', '她们', '它们', '我的', '我们的', '你的', '你们的', '他的', '她的', '它的', '他们的',
            '她们的', '它们的', '我自己', '我们自己', '你自己', '你们自己', '他自己', '她自己', '他们自己', '她们自己', '它们自己', '它自己', '我自己的', '这', '那',
            '这儿', '那儿', '这里', '那里', '这个', '那个', '这些', '那些', '这样', '那样', '这么', '那么', '##', '@@']
...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
    ...
    if data_type == 'text':
        ...
        i = 50002
        for tok in pro_list:
            if tok not in fields["src"].vocab.itos:
                fields["src"].vocab.itos.append(tok)
                fields["src"].vocab.stoi[tok]= i
                i = i + 1
...
def lazily_load_dataset(corpus_type, opt):
    ...
    pts = glob.glob(opt.data + '.' + corpus_type + '.[0-9]*.pt') # sorted(glob.glob(opt.data + '.' + corpus_type + '.[0-9]*.pt'))
    if pts:
        for pt in pts:
